...
private:
    ...
    void publishImuData()
    {
        ...
        // 6. GPS 辅助校正 (仅速度)
        // 将GPS速度从 km/h 转换为 m/s
        double gps_v_mps = current_imu_data.gps_v * 1000.0 / 3600.0;
        // 假设GPS速度是水平速度，且方向未知，这里只做简单的速度大小校正
        // 更复杂的融合需要GPS方向信息或卡尔曼滤波
        double current_horizontal_speed = std::sqrt(velocity_.x * velocity_.x + velocity_.y * velocity_.y);
        if (current_horizontal_speed > 0.1 && gps_v_mps > 0.1)
        { // 避免除以零或在静止时校正
            double speed_ratio = gps_v_mps / current_horizontal_speed;
            velocity_.x *= (1.0 - ACCEL_WEIGHT) + speed_ratio * ACCEL_WEIGHT;
            velocity_.y *= (1.0 - ACCEL_WEIGHT) + speed_ratio * ACCEL_WEIGHT;
        }
        else if (gps_v_mps < 0.1 && current_horizontal_speed < 0.1)
        {
            // 如果GPS和IMU都显示静止，则将速度清零以防止漂移
            velocity_.x = 0.0;
            velocity_.y = 0.0;
        }
        // 对于Z轴速度，由于没有GPS高度率，我们不进行直接校正，但可以通过其他方式（如气压计）辅助

        // 发布姿态和位置信息
        geometry_msgs::msg::PoseStamped pose_msg;
        pose_msg.header.stamp = this->now();
        pose_msg.header.frame_id = "map"; // 或者其他合适的坐标系
